...
if (!config.path) {
  console.error("\x1b[31m🚨 No S3 path configured. Please set a unique deploy path in project.config.js\x1b[0m 🚨")
  exit()
}

if (!live && !preview) {
  console.error(
    "The command 'npm run deploy' is deprecated. Please use 'npm run deploy:preview', or 'npm run deploy:live'",
  )
  exit()
}
...
deploy().catch((error) => {
  if (error.name === "InvalidToken") {
    spinner.fail(
      "Your AWS credentials are invalid. Please get a new set of credentials from Janus: https://janus.gutools.co.uk/",
    )
  } else if (error.name === "ExpiredToken") {
    spinner.fail(
      "Your AWS credentials have expired. Please get a new set of credentials from Janus: https://janus.gutools.co.uk/",
    )
  } else {
    spinner.fail()
    console.error(error)
...
async function deployAssets() {
  ...
  let paths = await listFiles(localAssetsPath, {
    filter: [".DS_Store", ".gitkeep"],
  })

  for (let filePath of paths) {
    if (filePath.endsWith(".json")) {
      // Check for urls pointing directly to S3
      const s3URL = "gdn-cdn.s3.amazonaws.com"
      const fileContents = fs.readFileSync(filePath, { encoding: "utf-8" })
      if (fileContents.includes(s3URL)) {
        throw new Error(
          `URLs pointing directly to S3 (${s3URL}) should not be deployed. Replace with CDN: 'interactive.guim.co.uk'. File: ${filePath}`,
        )
      }
    }
