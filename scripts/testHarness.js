...
import { normalizePath, createViteRuntime } from "vite"
...
export function testHarness() {
  ...
  const plugin = {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          if (req.url.includes("%3C%=%20path%20%%3E")) {
            console.log(
              "The use of <%= path %> is not supported in this template. Please use __assetsPath__ instead",
            )
          }
          return next(error)
        }
    ...
    async load(id) {
      ...
      } else if ((match = id.match(/^\/atoms\/[^\/]+\/([^\/]+)\/$/))) {
        // match '/atoms/{atom}/{template}/'
    ...
    async transformTemplateHTML(templateHTML, mainHTML, id) {
      if (id === "/") {
        ...
        return _.template(templateHTML)({
          ...
          paragraphStyle: config.placeholders.paragraphBefore
            ? "display: block;"
            : "display: none;",
