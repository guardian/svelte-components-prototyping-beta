...
// Muted state changes are handled by passing props to the Video component

// Captions are handled within the Video component

// Playback control is delegated to Video component instances via props

// External caption probing removed; Video component loads captions when available

// Legacy inline video playback helpers removed; control via props instead
        ...
        {#if video.vid === active}
          <div class="video-wrapper {video.display}">
            <Video
              ...
              srt={video.hasCaptions && video.subs ? `${url}/${video.subs}` : null}
